 ...
 * SurveyService
 *
 * SurveyService represents the model behind the search form about `dpadjogja\survey\models\SurveyService`.
 *
...
use dpadjogja\survey\models\SurveyService as SurveyServiceModel;

class SurveyService extends SurveyServiceModel
{
	/**
	 * {@inheritdoc}
	 */
	public function rules()
	{
		return [
			...
			[['service_name', 'creation_date', 'modified_date', 'updated_date', 'creationDisplayname', 'modifiedDisplayname'], 'safe'],
		];
	}
	...
	public function search($params, $column=null)
	{
		if(!($column && is_array($column)))
			$query = SurveyServiceModel::find()->alias('t');
		else
			$query = SurveyServiceModel::find()->alias('t')->select($column);
		$query->joinWith([
			// 'creation creation', 
			// 'modified modified'
		]);
		...
		$query->andFilterWhere(['like', 't.service_name', $this->service_name])
